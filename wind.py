...
class TurbWind(Wind):
    ...
    def __init__(self, surrounding_wind: Wind, TI = 0.1, T: float = 500) -> None:
        ...
        def next_even_square(n: float) -> int:
            """
            Smallest integer >= n that is a perfect square with even parity.
            i.e., (even integer)^2.
            """
        ...
        # enforce BOTH: square and even
        self.nx = next_even_square(self.Lx / dx_target)
        self.ny = next_even_square(Ly / dy_target)
        self.nz = next_even_square(Lz / dz_target)

        # Calculate resulting grid spacing
        self.dx = self.Lx / (self.nx - 1)
        self.dy = Ly / (self.ny - 1)
        self.dz = Lz / (self.nz - 1)

        print(f"[TurbWind] Nxyz=({self.nx}, {self.ny}, {self.nz}) | dxyz=({self.dx:.3f}, {self.dy:.3f}, {self.dz:.3f})")


        self.mann_dir = os.path.join(os.path.dirname(__file__), "sim_data", "mann_boxes")
        os.makedirs(self.mann_dir, exist_ok=True)

        # cache key components (so different T / box geometry creates different files)
        def _f(x, nd=3):
            return f"{x:.{nd}f}".rstrip("0").rstrip(".").replace(".", "p")
        ...
        box_key = (
            f"T{t_str}_"
            f"Lx{lx_str}_"
            f"N{self.nx}x{self.ny}x{self.nz}_"
            f"d{dx_str}_{dy_str}_{dz_str}"
        )

        # Generate/load a reference Mann box for this exact box geometry
        fname_ref = f"mann_box_reference_{box_key}.nc"
        ...
        # Scaled cache also includes same box_key + TI + U
        ti_str = _f(self.TI, 4)
        umean_str = _f(u_mean, 2)
        fname_scaled = f"mann_box_scaled_{box_key}_ti{ti_str}_umean{umean_str}.nc"
        fpath_scaled = os.path.join(self.mann_dir, fname_scaled)

        if not os.path.exists(fpath_scaled):
            print(f"Scaling Mann box for TI={self.TI} and v_hub={u_mean} m/s")
